...
module "eks" {
  ...
  enabled_log_types  = ["api", "controllerManager", "scheduler"]

  vpc_id     = var.vpc_id
  subnet_ids = var.private_subnets

  # v21: cluster_endpoint_private/public_access → endpoint_private/public_access
  endpoint_private_access = true
  endpoint_public_access  = true

  # v21: aws_auth_configmap replaced by access_entries (API-based RBAC)
  access_entries = {
    for user in var.aws_auth_users_list : user.userarn => {
  ...
  # v21: eks_managed_node_group_defaults removed; common settings go inline
...
resource "helm_release" "aws_efs_csi_driver" {
  ...
  set = [
    ...
    { name = "controller.serviceAccount.annotations.eks\\.amazonaws\\.com/role-arn", value = module.attach_efs_csi_role.iam_role_arn },
    { name = "controller.serviceAccount.name", value = "efs-csi-controller-sa" },
  ]
}
